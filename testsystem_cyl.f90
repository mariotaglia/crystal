integer function testsystem_cyl(x)
use system
use transform
use cylinder
...
if (v(1).le.0.0) then
 if (PBC(1).eq.2)testsystem_cyl = -1
 if (PBC(1).eq.0)testsystem_cyl = -2
endif

if (v(1).gt.(float(dimx)*delta)) then
 if (PBC(2).eq.2)testsystem_cyl = -1
 if (PBC(2).eq.0)testsystem_cyl = -2
endif

if (v(2).le.0.0) then
 if (PBC(3).eq.2)testsystem_cyl = -1
 if (PBC(3).eq.0)testsystem_cyl = -2
endif

if (v(2).gt.(float(dimy)*delta)) then
 if (PBC(4).eq.2)testsystem_cyl = -1
 if (PBC(4).eq.0)testsystem_cyl = -2
endif

if (v(3).le.0.0) then
 if (PBC(5).eq.2)testsystem_cyl = -1
 if (PBC(5).eq.0)testsystem_cyl = -2
endif

if (v(3).gt.(float(dimz)*delta)) then
 if (PBC(6).eq.2)testsystem_cyl = -1
 if (PBC(6).eq.0)testsystem_cyl = -2
endif

if (testsystem_cyl.eq.0) then ! saves some time
...
if (((x(1) - ctrans(1))**2 + (x(2) - ctrans(2))**2).lt.(r_cyl**2)) then
